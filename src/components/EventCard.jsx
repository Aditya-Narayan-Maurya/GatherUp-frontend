...
const EventCard = ({event_id,event_name,venue,date="",event_image_url}) => {
    ...
    console.log();
  return (
    <div className='group w-96 m-3 p-2 border border-[#b1aaaa] rounded-xl' key={event_id}>
      ...
      className='w-full h-52 p-2 rounded-2xl transition-transform duration-300 group-hover:scale-105 group-hover:skew-1'/>
       <div className='m-2'>
        ...
        <Link to={`/events/${event_id}`}> <button className='border rounded-2xl bg-[#EED3D9] hover:bg-transparent p-2 mt-2 w-full text-center cursor-pointer'>View Details</button></Link>
       </div>
    </div>
  )
}
